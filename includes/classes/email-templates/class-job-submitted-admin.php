 ...
 * do_action('jobsearch_job_submitted_admin', $user_data, $post_id);
 * 
 */
if (!class_exists('jobsearch_job_submitted_admin_template')) {

    class jobsearch_job_submitted_admin_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_job_submitted_admin_template_init'), 1, 0);
            add_filter('jobsearch_job_submitted_admin_filter', array($this, 'jobsearch_job_submitted_admin_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_job_submitted_admin', array($this, 'jobsearch_job_submitted_admin_callback'), 10, 2);
        }

        public function jobsearch_job_submitted_admin_template_init() {
            $this->user = array();
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'job_submitted_admin';
            $this->filter = 'job_submitted_admin';
            $this->email_template_db_id = 'job_submitted_admin';
            $this->switch_label = esc_html__('New Job submited to admin', 'wp-jobsearch');
            $this->default_subject = esc_html__('New Job found, Review that job', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_job_submitted_admin_filter', $default_content, 'html', 'job-submitted-admin', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'job_submitted_admin';
            $this->codes = array(// value_callback replace with function_callback tag replace with var
                ...
                array(
                    'var' => '{job_sectors}',
                    'display_text' => esc_html__('sector list', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_job_added_sector_list'),
                ),
                array(
                    'var' => '{job_sector}',
        ...
        public function jobsearch_job_submitted_admin_callback($user = '', $job_id = '') {
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : __('Job Added', 'wp-jobsearch');
                ...
                jobsearch_job_submitted_admin_template::$is_email_sent1 = $this->is_email_sent;
            }
        }

        public static function template_path() {
            return apply_filters('jobsearch_plugin_template_path', 'jobsearch-plugin/');
        }

        public function jobsearch_job_submitted_admin_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['job_submitted_admin']['rand'] = $this->rand;
            $email_template_options['job_submitted_admin']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['job_submitted_admin']['default_var'] = $this->default_var;
        ...
        public function get_job_added_publish_date() {
            ...
            $job_publish_date = isset($job_publish_date) && $job_publish_date != '' ? date($date_format, $job_publish_date) : '';
        ...
        public function get_job_added_expiry_date() {
            ...
            $job_expiry_date = isset($job_expiry_date) && $job_expiry_date != '' ? date($date_format, $job_expiry_date) : '';
    ...
    $jobsearch_job_submitted_admin_template = new jobsearch_job_submitted_admin_template();
    global $jobsearch_job_submitted_admin_template;
}
