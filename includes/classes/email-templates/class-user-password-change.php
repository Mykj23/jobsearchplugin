 ...
 * do_action('jobsearch_user_password_change', $user_data, $post_id);
 * 
 */
if (!class_exists('jobsearch_user_password_change_template')) {

    class jobsearch_user_password_change_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_user_password_change_template_init'), 1, 0);
            add_filter('jobsearch_user_password_change_filter', array($this, 'jobsearch_user_password_change_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_user_password_change', array($this, 'jobsearch_user_password_change_callback'), 10, 2);
            
            //
            add_filter('send_password_change_email', '__return_false');
        }

        public function jobsearch_user_password_change_template_init() {
            $this->user = array();
            $this->password = '';
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'user_password_change';
            $this->filter = 'user_password_change';
            $this->email_template_db_id = 'user_password_change';
            $this->switch_label = esc_html__('User Password Change', 'wp-jobsearch');
            $this->default_subject = esc_html__('User Password Change', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_user_password_change_filter', $default_content, 'html', 'user-password-change', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'user_password_change';
            $this->codes = array(
                ...
                array(
                    'var' => '{user_password}',
                    'display_text' => esc_html__('New Password', 'wp-jobsearch'),
        ...
        public function jobsearch_user_password_change_callback($user = '', $password = '') {
            global $sitepress;
            $lang_code = '';
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : __('Password Changed', 'wp-jobsearch');
                ...
                jobsearch_user_password_change_template::$is_email_sent1 = $this->is_email_sent;
            }
        }
        ...
        public function jobsearch_user_password_change_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['user_password_change']['rand'] = $this->rand;
            $email_template_options['user_password_change']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['user_password_change']['default_var'] = $this->default_var;
    ...
    new jobsearch_user_password_change_template();
}