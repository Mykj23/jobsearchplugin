 ...
 * do_action('jobsearch_new_user_register', $user_data, $post_id);
 * 
 */
if (!class_exists('jobsearch_new_user_register_template')) {

    class jobsearch_new_user_register_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_new_user_register_template_init'), 1, 0);
            add_filter('jobsearch_new_user_register_filter', array($this, 'jobsearch_new_user_register_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_new_user_register', array($this, 'jobsearch_new_user_register_callback'), 10, 2);
        }

        public function jobsearch_new_user_register_template_init() {
            $this->user = array();
            $this->user_password = '';
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'new_user_register';
            $this->filter = 'new_user_register';
            $this->email_template_db_id = 'new_user_register';
            $this->switch_label = esc_html__('To New User Register', 'wp-jobsearch');
            $this->default_subject = esc_html__('To New User Register', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_new_user_register_filter', $default_content, 'html', 'new-user-register', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'new_user_register';
            $this->codes = array(
        ...
        public function jobsearch_new_user_register_callback($user = '', $password = '') {
            global $sitepress;
            $lang_code = '';
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : __('Registeration Successfull', 'wp-jobsearch');
                ...
                jobsearch_new_user_register_template::$is_email_sent1 = $this->is_email_sent;
            }
        }
        ...
        public function jobsearch_new_user_register_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['new_user_register']['rand'] = $this->rand;
            $email_template_options['new_user_register']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['new_user_register']['default_var'] = $this->default_var;
    ...
    new jobsearch_new_user_register_template();
}